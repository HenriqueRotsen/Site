...
export async function cnpjHash(secret, cnpj) {
  const { hmacSha256Hex } = await import('./crypto.js');
  return hmacSha256Hex(secret, normalizeCnpj(cnpj));
}

export function cnpjLast4(cnpj) {
  return normalizeCnpj(cnpj).slice(-4);
}
